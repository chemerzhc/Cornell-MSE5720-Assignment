...
for para in {20..100..10}; do
    ...
    sed -i "s/PREFIX *= *\"CTO_dis_[0-9]\+\"/PREFIX=\"CTO_dis_${para}\"/" "$newfile"

    # Update results_folder path
    sed -i "s|results_folder *= *\$SCRATCH/cto_dis_[0-9]\+|results_folder=\$SCRATCH/cto_dis_${para}|" "$newfile"
