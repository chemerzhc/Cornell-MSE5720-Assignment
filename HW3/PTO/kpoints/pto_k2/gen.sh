...
for k in {1..12}; do
    ...
    # 修改 PREFIX
    sed -E -i "s|([[:space:]]*PREFIX[[:space:]]*=[[:space:]]*)['\"]?[^'\"[:space:]]+['\"]?|\1\"pto_k${k}\"|I" "$newfile"

    # 修改 results_folder
    sed -E -i "s|(results_folder[[:space:]]*=[[:space:]]*)\\\$SCRATCH/[^[:space:]]+|\1\\\$SCRATCH/pto_k${k}|I" "$newfile"
...
chmod +x main.sh
echo "✅ All pto_k*.sh scripts generated and main.sh created."
