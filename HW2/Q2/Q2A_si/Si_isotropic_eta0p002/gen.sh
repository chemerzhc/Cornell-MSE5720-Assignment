#!/bin/bash
# gen.sh — Generate Si deformation scripts (Exercise 2A)
...
# base lattice vectors in alat units (diamond/zinc-blende)
a1=(-0.5 0.0 0.5)
a2=(0.0 0.5 0.5)
a3=(-0.5 0.5 0.0)
...
for def in "${deformations[@]}"; do
  for eta in "${etas[@]}"; do
    ...
    newfile="Si_${def}_eta${eta_dir}.sh"
    cp "$TEMPLATE" "$newfile"

    # Replace PREFIX
    sed -E -i "s|^(PREFIX=).*|\1\"Si_${def}_eta${eta_dir}\"|I" "$newfile"

    # Replace results_folder
    sed -E -i "s|(results_folder=)\\\$SCRATCH/[^[:space:]]+|\1\\\$SCRATCH/Si_${def}_eta${eta_dir}|I" "$newfile"
...
chmod +x main.sh
echo "✅ All Si deformation scripts generated. Run ./main.sh to submit."
