...
for para in {20..150..10}; do
    ...
    sed -E -i "s|([[:space:]]*PREFIX[[:space:]]*=[[:space:]]*)['\"]?[^'\"[:space:]]+['\"]?|\1\"DIAMOND_${para}\"|I" "$newfile"

    # Replace results_folder = $SCRATCH/whatever  (handles paths with underscores)0
    sed -E -i "s|(results_folder[[:space:]]*=[[:space:]]*)\\\$SCRATCH/[^[:space:]]+|\1\\\$SCRATCH/diamond_${para}|I" "$newfile"
