...
for para in {20..100..10}; do
    ...
    sed -E -i "s|([[:space:]]*PREFIX[[:space:]]*=[[:space:]]*)['\"]?[^'\"[:space:]]+['\"]?|\1\"BTO_${para}\"|I" "$newfile"

    # Replace results_folder = $SCRATCH/whatever  (handles paths with underscores)
    sed -E -i "s|(results_folder[[:space:]]*=[[:space:]]*)\\\$SCRATCH/[^[:space:]]+|\1\\\$SCRATCH/bto_${para}|I" "$newfile"
